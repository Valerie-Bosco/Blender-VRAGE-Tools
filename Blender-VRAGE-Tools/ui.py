...
class VRT_PT_Panel(Panel):
    ...
    def draw(self, context):
        ...
        layout.operator(
            "scene.vrt_relink_project_materials",
            text="Fix Project Materials",
            icon="MATERIAL",
        )
...
class VRT_PT_Panel_subpanel_physics(Panel):
    ...
    def draw(self, context):
        ...
        layout.operator(
            "scene.vrt_export_collisions", text="Export Collisions", icon="EXPORT"
        )
        layout.separator()
        layout.operator(
            "scene.vrt_add_rigid_body", text="Add Rigid Body", icon="PHYSICS"
        )
        layout.operator(
            "object.vrt_convex_hull_from_selected",
            text="Generate Convex Hull",
            icon="MESH_ICOSPHERE",
        )
...
class VRT_PT_BlockProperties_subpanel_fractures(Panel):
    ...
    def draw(self, context):
        ...
        index_owner_path, index_prop_name = (
            "scene.vrt.fractures_list_active_index".rsplit(".", 1)
        )
        ...
        col.menu(
            "VRT_MT_Menu_subpanel_fractures_more_options", text="", icon="DOWNARROW_HLT"
        )
...
class VRT_MT_Menu_subpanel_fractures_more_options(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(
            "scene.vrt_fracture_repopulate_list",
            text="Repopulate List",
            icon="FILE_REFRESH",
        )
...
class VRT_PT_BlockProperties_subpanel_sections(Panel):
    ...
    def draw(self, context):
        ...
        index_owner_path, index_prop_name = (
            "scene.vrt.sections_list_active_index".rsplit(".", 1)
        )
        index_owner = context.path_resolve(index_owner_path)
        ...
        col.separator(factor=1.0)
        col.menu(
            "VRT_MT_Menu_subpanel_sections_more_options", text="", icon="DOWNARROW_HLT"
        )
...
class VRT_MT_Menu_subpanel_sections_more_options(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(
            "scene.vrt_section_repopulate_list",
            text="Repopulate List",
            icon="FILE_REFRESH",
        )
        layout.separator()
