...
class VRT_OT_ResetPaintColor(Operator):
    ...
    def execute(self, context):
        bpy.context.scene.VRT_paint_color_ui = (0.5, 0.5, 0.5)
        return {'FINISHED'}
...
class VRT_OT_section_add(Operator):
    ...
    def execute(self, context):
        my_list = context.scene.VRT_sections_list
        active_index = context.scene.VRT_sections_list_active_index

        to_index = min(len(my_list), active_index + 1)

        my_list.add()
        my_list.move(len(my_list) - 1, to_index)
        context.scene.VRT_sections_list_active_index = to_index
...
class VRT_OT_section_remove(Operator):
    ...
    def poll(cls, context):
        ...
        return len(context.scene.VRT_sections_list) > 0
...
class VRT_OT_Section_Assign(Operator):
    ...
    def execute(self, context):
        sections_list = bpy.context.scene.VRT_sections_list
        active_section_index = bpy.context.scene.VRT_sections_list_active_index
...
class VRT_OT_QuickExport(Operator):
    ...
    def poll(cls, context):
        ...
        dir_set = bool(context.scene.VRT_export_directory) # path not ""
        if dir_set: # if path is not empty string, check that it's valid
            dir_set = os.path.exists(context.scene.VRT_export_directory)
    ...
    def execute(self, context):
        ...
        dir = context.scene.VRT_export_directory
        var = context.scene.VRT_export_variant
        lod = self.export_lod
