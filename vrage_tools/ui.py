...
class VRT_PT_Materials_subpanel_info(Panel):

    bl_idname = 'VRT_PT_Materials_subpanel_info'
    bl_label = "Materials info"
    bl_parent_id = 'VRT_PT_Materials'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_options = {'DEFAULT_CLOSED'}
    ...
    def draw(self, context: bpy.types.Context):
        ...
        if (context.active_object is not None) and (context.active_object.type == "MESH"):
            material_count = len([0 for material_slot in context.active_object.material_slots if (material_slot.material is not None)])

            layout.label(text=f"Material count:{material_count}")
        else:
            layout.label(text=f"Object is not a Mesh")
